...
def update_book(book_name, updated_data):
    df = pd.read_excel('library_database.xlsx')
    index = df.index[df['Book Name'] == book_name].tolist()[0]
...
def main():
    ...
    if st.button('Search'):
        ...
        if not search_results.empty:
            ...
            updated_date_of_publication = st.date_input('Date of Publication', value=selected_book['Date of Publication'])
            ...
            if st.button('Update'):
                updated_data = {
                    ...
                    'Date of Publication': updated_date_of_publication,
                    'ISBN': updated_isbn,
